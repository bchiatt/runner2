...
Day.findById = function(id, cb){
  pg.query('select * from days_in_week where id = $1 limit 1', [id], function(err, results){
    cb(err, results && results.rows[0] ? results.rows[0] : null);
  });
};

Day.all = function(user, cb){
  pg.query('select * from days_in_week where org_id = $1', [user.org.id], function(err, results){
...
Day.add = function(user, obj, cb){
  var day = new Day(obj);
  pg.query('select day_add($1, $2, $3, $4)', [user.org.id, day.name, day.abbr, day.letter], function(err, results){
    cb(err, results && results.rows[0] ? results.rows[0] : null);
  });
};
